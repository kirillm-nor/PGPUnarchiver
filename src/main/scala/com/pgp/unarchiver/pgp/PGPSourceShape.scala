...
import com.pgp.unarchiver.pgp.PGPSourceShape.{
  Base64HeadersPGPNotSupported,
  Base64PGPNotSupported
}
...
object PGPSourceShape {
  ...
  def apply(bucket: String, fileKey: String, fileSize: Long)(
...
/**
  ...
  * @param system
  */
class PGPSourceShape(bucket: String, fileKey: String, fileSize: Long)(
    implicit materialiser: ActorMaterializer,
    system: ActorSystem)
    extends GraphStage[SourceShape[Done]] {
  ...
  override def createLogic(inheritedAttributes: Attributes): GraphStageLogic =
    new GraphStageLogic(shape) with StageLogging with OutHandler {
      ...
      private def isPossiblyBase64(ch: Byte) =
        (ch >= 'A' && ch <= 'Z') || (ch >= 'a' && ch <= 'z') || (ch >= '0' && ch <= '9') || (ch == '+') || (ch == '/') || (ch == '\r') || (ch == '\n')

      override def onPull(): Unit = {
        val bArrFuture = Source
          .fromFutureSource(
            S3.download(bucket, fileKey, Some(ByteRange(0, 60)))
              .runWith(Sink.head)
              .collect {
        ...
        log.debug("Received header of pgp archive")
        if ((arr.head & 0x80) != 0) {
          log.debug("Binary stream, continue processing")
          push(out, Done)
          complete(out)
        } else {
          log.error("Armored stream, dropping decryption")
          arr.partition(isPossiblyBase64) match {
            case (_, a) if a.isEmpty => failStage(Base64HeadersPGPNotSupported)
            case (c, _) if c.length < 4 =>
              failStage(Base64HeadersPGPNotSupported)
            case (c, _) =>
              val block = c.filter(ch => ch != '\n' && ch != '\r').take(8)
              if ((Base64.decode(block).head & 0x80) != 0)
                failStage(Base64PGPNotSupported)
